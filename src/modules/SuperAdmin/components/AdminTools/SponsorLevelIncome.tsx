...
export default function LevelSettingsManager({ }: Props) {
    ...
    const fetchLevels = async (type = incomeType) => {
        try {
            setTableLoading(true);

            // 1. Fetch Max Level
            const maxRes = await universalService({
                procName: "IncomeSetting",
                Para: JSON.stringify({
                    ActionMode: "GetMaxLevel",
                    IncomeName: type,
                }),
            });

            const maxData = maxRes?.data || maxRes;
            const maxLevel = Number(maxData?.[0]?.MaxLevel || 0);

            // 2. Fetch Existing Levels
            ...
            // 3. Create rows according to max level
            const generatedRows: LevelSetting[] = [];

            for (let i = 1; i <= maxLevel; i++) {

                const existing = data?.find(
                    (x: any) => Number(x.LevelNo) === i
                );

                generatedRows.push({
                    RequiredSponsorCount:
                        existing?.RequiredSponsorCount ?? "",

                    RequiredBusinessAmount:
                        existing?.RequiredBusinessAmount ?? "",

                    LevelPercentage:
                        existing?.LevelPercentage ?? "",
                });
            }

            setRows(generatedRows.length ? generatedRows : [emptyRow]);
