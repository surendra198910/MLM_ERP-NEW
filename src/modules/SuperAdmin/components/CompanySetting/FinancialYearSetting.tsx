...
const Template: React.FC = () => {
  ...
  const fetchGridColumns = async () => {
    ...
    try {
      const payload = {
        ...
        Para: JSON.stringify({
          UserId: employeeId,
          GridName: "USP_FinancialYearMaster",
        }),
      };
  ...
  const handleEdit = async (row: any) => {
    try {
      ...
      if (!data) throw new Error("No edit data");
  ...
  const handleDelete = async (row: any) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `Delete Financial Year "${row.FromDate} - ${row.ToDate}" ?`,
      icon: "warning",
  ...
  const fetchVisibleColumns = async () => {
    ...
    const payload = {
      ...
      Para: JSON.stringify({
        EmployeeId: employeeId,
        USPName: "USP_FinancialYearMaster",
        ActionMode: "List",
        Mode: "Get",
      }),
    };
    ...
    <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
      ...
      {!showTable && (
        <LandingIllustration
          ...
          description={
            <>
              Search Financial Year using filters above.<br />
              Manage records, export reports and analyse performance.<br />
              <span className="font-medium">OR</span><br />
              Click on Add button to create a new Financial entry.
            </>
          }
        />
      )}
...
p-[20px] md:p-[25px] rounded-t-md"
                ...
                {editLoading ? (
                  ...
                  <Formik
                    initialValues={{
                      FromDate: editData?.FromDate || todayDate,
                      ToDate: editData?.ToDate || todayDate,
                    ...
                    onSubmit={async (values, { resetForm }) => {
                      ...
                      const confirm = await Swal.fire({
                        ...
                        text: `Do you want to ${actionText} this financial year?`,
                        icon: "question",
                        showCancelButton: true,
                      ...
                      try {
                        ...
                        const payload = {
                          ...
                          Para: JSON.stringify({
                            ...
                            EditId: isEdit ? editData?.FinancialYearId : 0,
                            CompanyId: 1,
                            EntryBy: 1,
                            ModifiedBy: 1,
                          }),
