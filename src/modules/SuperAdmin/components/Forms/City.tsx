...
const ToDoList: React.FC = () => {
  ...
  const [sortColumn, setSortColumn] = useState("CityId");
  ...
  const fetchVisibleColumns = async () => {
    try {
      ...
      setVisibleColumns(
        cols
          .filter(
            (c) => c.IsVisible == 1 && c.IsHidden == 0,
            // c.ColumnName !== "CityId" // 🚫 HIDE DOCUMENT ID
          )
          .sort((a, b) => a.DisplayOrder - b.DisplayOrder),
      );
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res?.StatusCode === "1" || res?.StatusCode === 1) {
        ...
        ShowSuccessAlert("Unable to delete state");
      }
    } catch (err) {
 ...
 const fetchDocumentPermissions = async () => {
  try {
    ...
    const payload = {
      ...
      Para: JSON.stringify({
        ActionMode: "Forms",
        FormCategoryId: 2, // category of this component
        EmployeeId: employeeId,
      }),
    };
    ...
    // ❌ No page access if Action is empty / missing
    if (
  ...
  //     console.log("🧠 City Actions:", actions);
  //   }, 300);
  ...
  useEffect(() => {
    if (editData?.CountryId) {
      fetchStates(editData.CountryId);
    }
  }, [editData]);
          ...
          >
            ...
            <div className="md:max-w-md md:px-3 px-0 py-14">
              ...
              <p className="text-gray-600 dark:text-gray-300 leading-relaxed mb-6 text-[15px]">
                Search City data by <br />
                using above filters to edit, delete etc.
                <br />
                OR <br />
                Click Below to add New City
              </p>
                            ...
                            <div>
                              ...
                              <Field
                                ...
                                onChange={(e) => {
                                  ...
                                  setFieldValue("StateId", ""); // reset state
                                  if (countryId) {
      ...
      outline-0 cursor-pointer transition-all focus:border-primary-button-bg"
                            ...
                            <div>
                              ...
                              <Field
                                as="select"
                                name="StateId"
                                disabled={!states.length} // ✅ MUST be CountryId
                                className="h-[55px] rounded-md text-black dark:text-white border border-gray-200
      dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[14px] block w-full
      outline-0 cursor-pointer transition-all focus:border-primary-button-bg"
                              >
