...
const ToDoList: React.FC = () => {
  ...
  const handleAddNewCountry = () => {
    setIsEdit(false);
    setEditData(null); // 🔥 THIS IS THE KEY
    setSelectedIcon("");
    setIconColor("#1976d2");
    setOpen(true);
  };
  ...
  const [sortColumn, setSortColumn] = useState("CountryId");
  ...
  const fetchVisibleColumns = async () => {
    try {
      ...
      setVisibleColumns(
        cols
          .filter(
            (c) => c.IsVisible == 1 && c.ColumnName !== "CountryId", // 🚫 HIDE DOCUMENT ID
          )
          .sort((a, b) => a.DisplayOrder - b.DisplayOrder),
      );
    } catch (err) {
  ...
  const fetchEditData = async (id: number) => {
    ...
    try {
      ...
      if (!data || !data.CountryId) {
        throw new Error("Country edit data not found");
      }
 ...
 const fetchDocumentPermissions = async () => {
  ...
  //     console.log("🧠 Country Actions:", actions);
  //   }, 300);
              ...
              SmartActions.canAdvancedSearch(CURRENT_FORM_ID)
                ...
                : "bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed"
                    ...
                    >
                      ...
                      <option value="DocumentName">Country Name</option>
                    </select>
          ...
          >
            ...
            <div className="md:max-w-md md:px-3 px-0 py-14">
              ...
              <p className="text-gray-600 dark:text-gray-300 leading-relaxed mb-6 text-[15px]">
                Search Country data by <br />
                using above filters to edit, delete etc.
                <br />
                OR <br />
                Click Below to add New Country
              </p>

              <PermissionAwareTooltip
                allowed={SmartActions.canAdd(CURRENT_FORM_ID)}
                allowedText="Add New"
                deniedText="You do not have permission to perform this action"
              >
                <button
                  type="button"
                  onClick={() => {
...
p-[20px] md:p-[25px] rounded-t-md"
                  ...
                  <Formik
                    initialValues={{
                      CountryName:
                        isEdit && editData ? editData.CountryName : "",
                      CountryCode:
                        isEdit && editData ? editData.CountryCode : "",
                      CountryFlag:
                        isEdit && editData ? editData.CountryFlag : "",
                      Language: isEdit && editData ? editData.Language : "",
                    }}
                    enableReinitialize
                      ...
                      try {
                        ...
                        const payload = {
                          ...
                          Para: JSON.stringify({
                            ...
                            EntryBy: 1,
                            EditId: isEdit ? Number(editData.CountryId) : 0,
                          }),
                        };
                        ...
                        if (res?.StatusCode === 1) {
                          ShowSuccessAlert(
                            res.msg || "Country Saved Successfully",
                          );
