...
const ToDoList: React.FC = () => {
 ...
 const [permissionsLoading, setPermissionsLoading] = useState(true);
  ...
  const exportCSV = async () => {
    ...
    a.href = url;
    a.download = "client_ContactType.csv";
    a.click();
  };
  ...
  const exportExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Client ContactTypeName");
    XLSX.writeFile(workbook, "client_ContactType.xlsx");
  };
  ...
  const exportPDF = async () => {
    ...
    doc.save("client_ContactType.pdf");
  };
  ...
  const [sortColumn, setSortColumn] = useState("ContactTypeId");
    ...
    try {
      ...
      const payload = {
        ...
        Para: JSON.stringify({
          ...
          USPName: "USP_ContactTypeMaster", // ✅ FIXED
          ActionMode: "List",
          Mode: "Get",
        }),
      };
      ...
      setVisibleColumns(
        cols
          .filter(
            (c) => c.IsVisible == 1 && c.ColumnName !== "ContactTypeId", // 🚫 HIDE DOCUMENT ID
          )
          .sort((a, b) => a.DisplayOrder - b.DisplayOrder),
      );
    } catch (err) {
  ...
  const fetchTableData = async () => {
    try {
      ...
      const payload = {
        procName: "ContactTypeMaster", // ✅ FIX
        Para: JSON.stringify({
          ActionMode: "List",
          CompanyId: 1,
          EntryBy: 1,
      ...
      setDataLoaded(true);
      // console.log("Table data fetched",tasks);
    }
  };
  ...
  const formSchema = Yup.object().shape({
    ContactTypeName: Yup.string().required("Contact Type is required"),
  });
 ...
 const fetchDocumentPermissions = async () => {
  try {
    ...
    const payload = {
      ...
      Para: JSON.stringify({
        ActionMode: "Forms",
        FormCategoryId: 2, // category used by this component
        EmployeeId: employeeId,
      }),
    };
    ...
    // ❌ No permission if Action is empty / null
    if (
      !pagePermission ||
    ...
    // ✅ Permission OK → load SmartActions
  ...
  //     SmartyActionStoreInstance.set("ClientContactType", actions);
  //     console.log("🧠 Client Document Actions:", actions);
  //   }, 300);
      ...
      <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
        <div className="trezo-card-header mb-[10px] md:mb-[10px] sm:flex items-center justify-between pb-5 border-b border-gray-200 -mx-[20px] md:-mx-[25px] px-[20px] md:px-[25px]">
          <div className="trezo-card-title">
            <h5 className="!mb-0">Manage Contact Type</h5>
          </div>
          ...
          >
            ...
            <div className="md:max-w-md md:px-3 px-0 py-14">
              ...
              <p className="text-gray-600 dark:text-gray-300 leading-relaxed mb-6 text-[15px]">
                Search Contact Type data by <br />
                using above filters to edit, delete etc.
                <br />
                OR <br />
                Click Below to add Contact Type
              </p>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (!SmartActions.canEdit(CURRENT_FORM_ID))
                                      return;
                                    fetchEditData(task.ContactTypeId);
                                  }}
                                  className={
                                    SmartActions.canEdit(CURRENT_FORM_ID)
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (
                                      ...
                                      return;
                                    handleDelete(task.ContactTypeId);
                                  }}
                                  className={
                                    SmartActions.canDelete(CURRENT_FORM_ID)
                    ...
                    onSubmit={async (values, { resetForm }) => {
                      try {
                        ...
                        if (res?.StatusCode === "1" || res?.StatusCode === 1) {
                          ...
                          setIsEdit(false);
                          setCurrentPage(1);
                          setSearchTrigger((p) => p + 1); // 🔁 refresh table
                        } else {
...
focus:border-primary-button-bg"
                          ...
                          <div className="text-right mt-[20px]">
                            <button
                              ...
                              onClick={() => {
                                resetForm(); // ✅ clears form values
                                setEditData(null); // ✅ clears edit state
