...
const ExpenditureGroup: React.FC = () => {
    const formSchema = Yup.object().shape({
        ExpenditureGroupName: Yup.string().required("Expenditure group name is required"),
    });
    ...
    const [sortColumnKey, setSortColumnKey] = useState<string>("ExpenditureGroupId");
        ...
        try {
            ...
            if (Array.isArray(data)) {
                ...
                const reactCols = visibleSorted
                    .filter((c: any) => c.ColumnKey !== "ExpenditureGroupId") // Hide primary key
                    .map((c: any) => ({
                        id: c.ColumnOrder,
                        name: c.DisplayName,
                        sortable: true,
    ...
    const handleEdit = async (row) => {
        ...
        try {
            const payload = {
                ...
                Para: JSON.stringify({
                    ActionMode: "Select",
                    EditId: Number(row.ExpenditureGroupId),
                }),
            };
    ...
    const fetchExportData = async () => {
        const payload = {
            ...
            Para: JSON.stringify({
                ...
                Page: page,
                PageSize: 0, // 0 skips OFFSET FETCH in SP, returning all filtered rows
                SortIndexColumn: sortColumnKey || "ExpenditureGroupId",
    ...
    const handleDelete = async (row: any) => {
        const result = await Swal.fire({
            title: "Are you sure?",
            text: `Delete "${row.ExpenditureGroupName}" ?`,
            icon: "warning",
        ...
        try {
            const payload = {
                ...
                Para: JSON.stringify({
                    ActionMode: "Delete",
                    EditId: row.ExpenditureGroupId,
                    EntryBy: 1, // SP maps this to ModifiedBy for deletion auditing
                }),
            };
    ...
    return (
        ...
        ${SmartActions.canSearch(formName)
            ...
            {!showTable && (
                <LandingIllustration
                    title="Manage Expenditure Group"
                    addLabel="Add Expenditure Group"
                    formName={formName}
                    description={
                        <>
                            Search expenditure group using filters above.<br />
                            Manage records, export reports and analyse performance.<br />
                            <span className="font-medium">OR</span><br />
                            Click on Add button to create a new expenditure group.
                        </>
                    }
                />
            )}
                        ...
                        <div className="flex justify-between items-center py-2 mb-[10px]">
                            ...
                            <PermissionAwareTooltip allowed={canExport}>
                                <div className={!canExport ? "pointer-events-none opacity-50" : ""}>
                                    <ExportButtons
                                        title="Expenditure Group Report"
                        ...
                        >
                            <div className="trezo-card w-full bg-white dark:bg-[#0c1427] p-[20px] md:p-[25px] rounded-md">
                                ...
                                >
                                    <div className="trezo-card-title">
                                        <h5 className="!mb-0">
                                            {isEdit ? "Edit Expenditure Group" : "Add New Expenditure Group"}
                                        </h5>
                                    </div>
                                    <button
                                        type="button"
                                ...
                                {editLoading ? (
                                    ...
                                    <Formik
                                        initialValues={{
                                            ExpenditureGroupName: editData?.ExpenditureGroupName || "",
                                        }}
                                        enableReinitialize
                                            ...
                                            try {
                                                ...
                                                const payload = {
                                                    ...
                                                    Para: JSON.stringify({
                                                        ...
                                                        EditId: isEdit ? Number(editData?.ExpenditureGroupId) : 0,
                                                        EntryBy: 1,
                                                        ModifiedBy: 1 // Passed correctly for the Update SP audit mapping
                                                    }),
                                                };
...
export default ExpenditureGroup;