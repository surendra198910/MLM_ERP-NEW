...
type Task = {
  ExpenditureGroupId: number;
  ExpenditureGroupName: string;
  Rate: number;
...
const [hasPageAccess, setHasPageAccess] = useState(true);
  ...
  const exportCSV = async () => {
    ...
    a.href = url;
    a.download = "expenditure_group.csv";
    a.click();
  };
  ...
  const exportExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Expenditure Group");
    XLSX.writeFile(workbook, "expenditure_group.xlsx");
  };
  ...
  const exportPDF = async () => {
    ...
    doc.save("expenditure_group.pdf");
  };
  ...
  const [sortColumn, setSortColumn] = useState("ExpenditureGroupId");
    ...
    try {
      ...
      const payload = {
        ...
        Para: JSON.stringify({
          ...
          USPName: "USP_ExpenditureGroup", // ✅ FIXED
          ActionMode: "List",
          Mode: "Get",
        }),
      };
      ...
      setVisibleColumns(
        cols
          .filter(
            (c) => c.IsVisible == 1 && c.ColumnName !== "ExpenditureGroupId", // 🚫 HIDE DOCUMENT ID
          )
          .sort((a, b) => a.DisplayOrder - b.DisplayOrder),
      );
    } catch (err) {
  ...
  const fetchTableData = async () => {
    try {
      ...
      setTasks(
        apiRes.map((row) => ({
          ExpenditureGroupId: row.ExpenditureGroupId,
          ExpenditureGroupName: row.ExpenditureGroupName,
          Rate: row.Rate,
  ...
  const fetchEditData = async (id: number) => {
    ...
    try {
      ...
      if (!data || !data.ExpenditureGroupId) {
        throw new Error("Edit data not found");
      }

      setEditData({
        id: data.ExpenditureGroupId,
        ExpenditureGroupName: data.ExpenditureGroupName,
 ...
 const fetchDocumentPermissions = async () => {
  ...
  //     SmartyActionStoreInstance.set("ClientDocuments", actions);
  //     console.log("🧠 Expenditure Group Actions:", actions);
  //   }, 300);
  ...
  useEffect(() => {
    if (!visibleColumns.some((c) => c.ColumnName === sortColumn)) {
      setSortColumn("ExpenditureGroupId");
      ...
      <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
        <div className="trezo-card-header mb-[10px] md:mb-[10px] sm:flex items-center justify-between pb-5 border-b border-gray-200 -mx-[20px] md:-mx-[25px] px-[20px] md:px-[25px]">
          <div className="trezo-card-title">
            <h5 className="!mb-0">Manage Expenditure Group</h5>
          </div>
              ...
              SmartActions.canAdvancedSearch(CURRENT_FORM_ID)
                ...
                : "bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed"
                    ...
                    >
                      ...
                      <option value="ExpenditureGroupName">
                        Expenditure Group Name
                      </option>
                    </select>
      ...
      `}
                    ...
                    <tbody className="text-black dark:text-white">
                      {displayedTasks.map((task: any) => (
                        <tr key={task.ExpenditureGroupId}>
                          {visibleColumns.map((col) => (
                            <td
                              key={col.ColumnName}
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (!SmartActions.canEdit(CURRENT_FORM_ID))
                                      return;
                                    fetchEditData(task.ExpenditureGroupId);
                                  }}
                                  className={
                                    SmartActions.canEdit(CURRENT_FORM_ID)
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (
                                      ...
                                      return;
                                    handleDelete(task.ExpenditureGroupId);
                                  }}
                                  className={
                                    SmartActions.canDelete(CURRENT_FORM_ID)
...
p-[20px] md:p-[25px] rounded-t-md"
                  >
                    <div className="trezo-card-title">
                      <h5 className="!mb-0">
                        {isEdit
                          ? "Edit Expenditure Group"
                          : "Add New Expenditure Group"}
                      </h5>
                    </div>
                    <button
                      type="button"
                  ...
                  {editLoading ? (
                    ...
                    <Formik
                      initialValues={{
                        ExpenditureGroupName:
                          editData?.ExpenditureGroupName || "",
                      ...
                      onSubmit={async (values, { resetForm }) => {
                        try {
                          ...
                          const payload = {
                            procName: "ExpenditureGroup",
                            Para: JSON.stringify({
                              ExpenditureGroupName: values.ExpenditureGroupName,
                          ...
                          if (res?.StatusCode === "1") {
                            ShowSuccessAlert(
                              isEdit
                                ? "Expenditure Group Updated Successfully"
                                : "Expenditure Group Added Successfully",
                            );
