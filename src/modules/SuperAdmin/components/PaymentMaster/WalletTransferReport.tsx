...
const Template: React.FC = () => {
  ...
  const fetchExportData = async () => {
    const payload = {
      ...
      Para: JSON.stringify({
        ActionMode: "GetTransferHistory",
        UserName: filterColumn === "UserName" ? searchInput : "",
        Status: "",
        Page: page,
        PageSize: 0,
  ...
  const fetchGridData = async (options?: any) => {
    ...
    try {
      ...
      if (Array.isArray(result)) {
        ...
        setData(result);
        setTotalRows(result.length ? result[0].TotalRecords : 0);
  ...
  const applySearch = () => {
    ...
    if (filterColumn && !searchInput.trim()) {
    ...
    <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
      ...
      {!showTable && (
        <LandingIllustration
          ...
          description={
            <>
              Search Wallet Transfer using filters above.<br />
