...
const Template: React.FC = () => {
  ...
  const fetchGridColumns = async () => {
    ...
    try {
      const payload = {
        ...
        Para: JSON.stringify({
          UserId: employeeId,
          GridName: "USP_EmployeeTypeMaster",
        }),
      };
  ...
  const handleDelete = async (row: any) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `Delete "${row.EmployeeTypeId}" ?`,
      icon: "warning",
  ...
  const fetchVisibleColumns = async () => {
    ...
    const payload = {
      ...
      Para: JSON.stringify({
        EmployeeId: employeeId,
        USPName: "USP_EmployeeTypeMaster",
        ActionMode: "List",
        Mode: "Get",
      }),
    };
    ...
    <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
            ...
            <div className="relative w-full sm:w-[180px]">
              ...
              >
                ...
                >
                  ...
                  <option value="LoginType">Employee Type</option>
      ...
      {!showTable && (
        <LandingIllustration
          ...
          description={
            <>
              Search Employee type using filters above.<br />
              Manage records, export reports and analyse performance.<br />
              <span className="font-medium">OR</span><br />
              Click on Add button to create a new Employee type entry.
            </>
          }
        />
      )}
                ...
                {editLoading ? (
                  ...
                  <Formik
                    initialValues={{
                      ...
                      Status:
                        String(editData?.Status).toLowerCase() === "active",
                    }}
                    enableReinitialize
                    validationSchema={formSchema}
                    onSubmit={async (values, { resetForm }) => {
                      const confirm = await Swal.fire({
                        title: isEdit ? "Update Employee Type?" : "Add Employee Type?",
                      ...
                      try {
                        ...
                        const payload = {
                          ...
                          Para: JSON.stringify({
                            ...
                            Status: values.Status ? 1 : 0, // ✅ FIXED
                            ActionMode: isEdit ? "Update" : "Insert", // ✅ FIXED
                            EditId: isEdit ? Number(editData?.EmployeeTypeId) : 0, // ✅ FIXED
                            CompanyId: 1,
                            EntryBy: 1,
                          }),
                        };
