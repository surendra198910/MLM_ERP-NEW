...
const ToDoList: React.FC = () => {
    ...
    const fetchBinaryIncome = async () => {
        try {
            ...
            const parsed = JSON.parse(apiRes.BinaryResultJson);
    ...
    const fetchExportData = async (): Promise<BinaryRecord[]> => {
        try {
            const payload = {
                procName: "ProcessBinaryIncome",
                Para: JSON.stringify({
    ...
    const exportCSV = async () => {
        ...
        const header = Binary_COLUMNS.map(col => col.label).join(",");

        const rows = data.map(row =>
            Binary_COLUMNS
                .map(col => `"${row[col.key as keyof BinaryRecord] ?? "-"}"`)
                .join(",")
        );
        ...
        a.href = url;
        a.download = "process-Binary-income.csv";
        a.click();
    };
    ...
    const exportExcel = async () => {
        ...
        const rows = data.map(row => {
            const obj: any = {};
            Binary_COLUMNS.forEach(col => {
                obj[col.label] = row[col.key as keyof BinaryRecord] ?? "-";
            });
            return obj;
        });
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Binary Income");
        XLSX.writeFile(workbook, "process-Binary-income.xlsx");
    };
    ...
    const exportPDF = async () => {
        try {
            ...
            const tableColumn = Binary_COLUMNS.map(col => col.label);

            const tableRows = data.map(row =>
                Binary_COLUMNS.map(col => row[col.key as keyof BinaryRecord] ?? "-")
            );
            ...
            doc.save("process-Binary-income.pdf");
    ...
    const handlePrint = async () => {
        ...
        const tableHeaders = Binary_COLUMNS
        ...
        const tableRows = data
            ...
            <tr>
                ${Binary_COLUMNS
                    .map(col => `<td>${row[col.key as keyof BinaryRecord] ?? "-"}</td>`)
                    .join("")}
            </tr>
        `)
            .join("");
        ...
        <html>
        <head>
            <title>Process Binary Income</title> 
            <style>
    ...
    const Binary_COLUMNS = [
        ...
        { key: "BinaryPercentage", label: "Binary %" },
    ...
    const handleProcessBinary = async (date: string) => {
        try {
            ...
            const payload = {
                procName: "DistributeInvestmentBinary",
                Para: JSON.stringify({
                            ...
                            `}
                                    ...
                                    <tbody className="text-black dark:text-white">
                                        {sortedRecords.map((row) => (
                                            ...
                                            >
                                                {Binary_COLUMNS.map((col) => {
                                                    const value = row[col.key as keyof BinaryRecord];
