...
const Template: React.FC = () => {
  const formSchema = Yup.object().shape({
    ...
    CountryId: Yup.string().required("Country is required"), // Changed to string to handle initial empty selection properly
  });
  ...
  const fetchCountryList = async () => {
    try {
      const payload = {
        procName: "Country", // Adjust this if your procName is different
        Para: JSON.stringify({
  ...
  const fetchGridColumns = async () => {
    ...
    try {
      const payload = {
        ...
        Para: JSON.stringify({
          UserId: employeeId,
          GridName: "USP_CompanyDocumentsMaster",
        }),
      };
  ...
  const handleEdit = async (row) => {
    ...
    try {
      const payload = {
        procName: "CompanyDocumentsMaster",
        Para: JSON.stringify({
          ActionMode: "Select",
      ...
      const res = await universalService(payload);
      const detailData = Array.isArray(res?.data || res)
        ? (res?.data || res)[0]
        : res?.data || res;
  ...
  const fetchVisibleColumns = async () => {
    ...
    const payload = {
      ...
      Para: JSON.stringify({
        EmployeeId: employeeId,
        USPName: "USP_CompanyDocumentsMaster",
        ActionMode: "List",
        Mode: "Get",
      }),
    };
  ...
  useEffect(() => {
    ...
    fetchCountryList(); // Trigger country fetch on mount
  }, []);
    ...
    <div className="trezo-card bg-white dark:bg-[#0c1427] mb-[25px] p-[20px] md:p-[25px] rounded-md">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 flex-wrap justify-end">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                >
                  <ColumnSelector
                    procName="USP_CompanyDocumentsMaster"
      ...
      {!showTable && (
        <LandingIllustration
          ...
          addLabel="Add Company Document"
          formName={formName}
          description={
            <>
              Search Company Documents using filters above.
              <br />
              Manage records, export reports and analyse performance.
              <br />
              <span className="font-medium">OR</span>
              <br />
              Click on Add button to create a new document entry.
            </>
          }
        />
      )}
      ...
      {showTable && (
        ...
        transition-all appearance-none"
              ...
              <PermissionAwareTooltip allowed={canExport}>
                ...
                >
                  <ExportButtons
                    title="Company Documents Report"
...
p-[20px] md:p-[25px] rounded-t-md"
                ...
                {editLoading ? (
                  ...
                  <Formik
                    initialValues={{
                      DocumentName:
                        isEdit && editData ? editData.DocumentName : "",
                      CountryId: isEdit && editData ? editData.CountryId : "",
                      IsAttachment:
                        isEdit && editData
                          ? editData.IsAttachment === "Yes" ||
                            editData.IsAttachment === true ||
                            editData.IsAttachment === 1
                          : false,
                    }}
                    enableReinitialize
                    ...
                    onSubmit={async (values, { resetForm }) => {
                      ...
                      try {
                        setSaving(true);
                        const saved = localStorage.getItem("EmployeeDetails");
                        const employeeId = saved
                          ? JSON.parse(saved).EmployeeId
                          : 0;
                        ...
                        const payload = {
                          ...
                          Para: JSON.stringify({
                            ...
                            IsAttachment: values.IsAttachment ? 1 : 0,
                            ActionMode: isEdit ? "Update" : "Insert",
                            EntryBy: employeeId,
                            EditId: isEdit ? Number(editData?.DocumentId) : 0,
                          }),
                        };
                        ...
                        if (
                          ...
                          resetForm();
                          closeModal(); // Ensure closeModal is defined, or use setOpen(false)
                          ...
                          Swal.fire(
                            "Error",
                            res?.Msg || res?.msg || "Operation failed",
                            "error",
                          );
                        }
                      } catch (err) {
...
focus:border-primary-button-bg"
                          ...
                          <div className="relative">
                            ...
                            <span className="absolute right-4 top-[45px] flex items-center pointer-events-none text-gray-500">
                              <i className="material-symbols-outlined !text-[20px]">
                                expand_more
                              </i>
                            </span>
                              ...
                              <Field name="IsAttachment">
                                {({ field }) => (
                                  <label className="relative inline-flex items-center cursor-pointer">
                                    ...
                                    />
                                    <div className="w-11 h-6 bg-gray-200 dark:bg-[#172036] rounded-full peer-checked:bg-primary-button-bg"></div>
