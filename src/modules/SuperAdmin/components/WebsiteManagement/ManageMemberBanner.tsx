...
const validationSchema = Yup.object().shape({
    imagePath: Yup.string().required("Banner image is required"),
});

export default function AddMemberBannerImage() {
    ...
    const handleCroppedImage = async (croppedBase64: string) => {
        try {
            ...
            await autoSaveBanner(uploadedFileName);
    ...
    const handleDelete = async (img) => {
        ...
        const confirm = await Swal.fire({
            ...
            text: "This banner will be removed.",
            icon: "warning",
    ...
    const handleSubmit = async (
        ...
        const confirm = await Swal.fire({
            title: isEditMode ? "Update Banner?" : "Add Banner?",
            icon: "question",
            showCancelButton: true,
        });
        ...
        try {
            ...
            const payload = {
                ...
                Para: JSON.stringify({
                    ...
                    MemberBannerImageId: id ? Number(id) : 0,
                    ImagePath: imagePath,
                    EntryBy: employeeId,
                    ModifiedBy: employeeId,
                }),
    ...
    const autoSaveBanner = async (uploadedFileName: string) => {
        ...
        try {
            ...
            if (res?.StatusCode === 1) {
                ...
                toast.success(res.Msg || "Banner Uploaded");
                ...
                toast.error(res?.Msg || "Failed to save banner");
                ...
                >
                    ...
                    <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-4 py-4">
                        <div className="text-lg font-bold text-gray-800 dark:text-white">
                            {isEditMode
                                ? "Edit Member Panel Banner Image"
                                : "Manage Member Panel Banner Image"}
                        </div>
                        ...
                        <div className="flex gap-x-2">
                            ...
                            {/* <PermissionAwareTooltip
                                allowed={isEditable}
                                allowedText={isEditMode ? "Update Banner" : "Add Banner"}
                                deniedText="Permission required"
                            >
                                <button
                                    type="submit"
                                    disabled={!isEditable || loading || isSubmitting}
                                    className={`px-4 py-1.5 rounded text-sm font-medium ${!isEditable ? "bg-gray-300 text-white cursor-not-allowed" : "bg-primary-button-bg hover:bg-primary-button-bg-hover text-white"}`}
                                >
                                ...
                                <div className="relative w-36 h-36 group">
                                    ...
                                    <div className="w-full h-full rounded-xl border-[4px] border-white dark:border-gray-700 shadow-md overflow-hidden bg-gray-200 dark:bg-gray-800 flex items-center justify-center relative">
                                        ...
                                        {image ? (
                                            ...
                                            <img src={`${IMAGE_PREVIEW_URL}${imagePath}`} className="w-full h-full object-cover" />
                                        ) : (
...
${img.Status === "Active"
                                                    ...
                                                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                                                        ...
                                                        {/* VIEW */}
                                                        <a
                                                            href={`${IMAGE_PREVIEW_URL}${img.ImagePath}`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="p-1.5 rounded-md bg-white/80 backdrop-blur hover:bg-blue-500 hover:text-white text-gray-700 shadow-sm transition"
                                                        >
                                                            <FaEye size={14} />
                                                        </a>
...
${img.Status === "Active" ? "bg-green-500" : "bg-gray-300"}`}
                                                        >
                                                            <span
                                                                className={`inline-block h-4 w-4 transform rounded-full bg-white transition
${img.Status === "Active" ? "translate-x-5" : "translate-x-1"}`}
                                                            />
                                                        </button>
                                                    </div>
