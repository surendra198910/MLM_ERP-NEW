...
export default function PairIncomeSetting({}: Props) {
  ...
  const addRow = () => {
    ...
    const newFromPair =
      lastRow && lastRow.ToPair ? parseInt(lastRow.ToPair) + 1 : "";

    setRows([
      ...rows,
      {
        FromPair: newFromPair,
        ToPair: "",
        Ratio: "",
        IncomePercentage: "",
        Capping: "",
  ...
  const handleSave = async () => {
    if (!SmartActions.canEdit(formName)) {
      Swal.fire(
        "Permission Denied",
        "You cannot update Pair Income settings.",
        "error",
      );
      return;
    }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-primary-table-bg text-primary-table-text dark:bg-[#15203c]">
              <th className="px-4 py-3 text-left font-semibold w-[80px]">
                S. No.
              </th>
              ...
              <th className="px-0 py-3 text-center font-semibold w-[100px]">
                Action
              </th>
            </tr>
          </thead>
